...
function renderDashboard(rows) {{
  ...
  DATA.forEach(p => {{
    ...
    let dot, label;
    if (nfTbls.length === 0) {{ dot = 'status-green'; label = 'Covered'; }}
    else if (coveredCount > 0) {{ dot = 'status-yellow'; label = 'Partial'; }}
...
function exportCoverageCSV() {{
  ...
  DATA.forEach(p => {{
    ...
    const status = nf ? (act || cfg ? 'Partial' : 'No Coverage') : 'Covered';
...
function printReport() {{
  ...
  DATA.forEach(p => {{
    ...
    const hasGap = nf.length > 0;
    const hasCov = act.length > 0 || cfg.length > 0;
    const cls = !hasGap ? 'green' : hasCov ? 'yellow' : 'red';
    const label = !hasGap ? 'Covered' : hasCov ? 'Partial' : 'None';
